...
def main():
    ...
    logger.configure(
        dir=DIR, mode="-".join(["evaluation", args.mode_evaluation]), args=args, tag="")
    ...
    model.load_state_dict(
        dist_util.load_state_dict(
            osp.join(DIR, model_path), map_location="cpu")
    )
    ...
    run_bpd_evaluation(model, loader, args.num_samples,
                       args.clip_denoised, args)
...
def run_bpd_evaluation(model, data, num_samples, clip_denoised, args):
    ...
    while num_complete < num_samples:
        ...
        if args.model == "ddpm":
            ...
            perm = th.LongTensor(
                [np.random.permutation([0, 1, 2, 3, 4])]).squeeze()
            c_x_set = x_set[:, perm]
        ...
        for key, term_list in all_metrics.items():
            terms = minibatch_metrics[key].mean(
                dim=0)  # / dist.get_world_size()
    ...
    for name, terms in all_metrics.items():
        if args.transfer:
            ...
            ), f"full_{name}_{args.mode_evaluation}_{args.sample_size}_transfer_{args.dataset}_terms.npz")
        else:
            out_path = os.path.join(logger.get_dir(
            ), f"full_{name}_{args.mode_evaluation}_{args.sample_size}_terms.npz")
